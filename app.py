...
def parse_scientific_report(text):
    """
    MASTER PARSER: The "Nuclear Safety Net" for Short Sellers.
    Forces correct scoring even if AI hallucinates risk on profitable shorts.
    """
    ...
    # ====================================================
    # ☢️ NUCLEAR LOGIC PATCH: SHORT SELLER PROTECTION
    # ====================================================
    # If it is a SHORT position...
    if "short" in data["type"]:
        combined_text_lower = (data["tech"] + data["risk"]).lower()
        
        # ...and we see "Support Break" or "Price Drop" (Good for shorts)
        triggers = ["support break", "price drop", "breaking down", "below support"]
        if any(t in combined_text_lower for t in triggers):
            
            # FORCE WIN STATUS
            data["outcome"] = "win" 
            
            # NUKE "LOSS" WORDS via REGEX
            # Replaces "indicating a loss", "risk of loss", "potential loss" -> "PROFIT POTENTIAL"
            # This handles variations like "indicating a potential loss" or "risk of further losses"
            ...
            # Specific fix for the "Critical Point" sentence often generated by AI
    ...
    if not is_winning_trade:
        ...
        # Only penalize squeeze if it's explicitly mentioned as a risk
    ...
    if is_winning_trade:
        # FORCE HIGH SCORE FOR WINNING SHORTS (Nuclear Override)
        score = max(score, 95) 
    else:
        ...
        if "Visual Evidence" in mode:
            ...
            if up_file:
                ...
                if st.button("INITIATE FORENSIC SCAN", type="primary"):
                    ...
                    # Forces the AI to TRY to be correct, but Python acts as the fallback.
                    prompt = f"""
